...
- `horsePositions.js`: Defines the `keyFrames` array, which stores the absolute positions of each horse (in meters) at specific one-second intervals.
...
-   **`y` position (pixels):** Represents the horse's vertical position on the canvas. These values are pixel coordinates and are interpolated between keyframes, allowing horses to change their vertical alignment.

The `script.js` file interpolates horse positions between the `keyFrames` data to create smooth movement. It then dynamically adjusts the view to focus on the leading horses, displaying their relative positions on the canvas.
